...
def check():
    ...
    for t in recent:
        print()
        print(f'--- Task: {t.task_id[:12] if t.task_id else "??"}... ---')
        ...
        if t.status == 'completed':
            ...
            # New: .part > total
            if actual_part is not None and t.total_bytes and t.total_bytes > 0:
                if actual_part > t.total_bytes:
                    issues.append(f'BAD: part_size {actual_part} > total {t.total_bytes} - file corrupt')
                elif actual_part < t.total_bytes and actual_part == t.downloaded_bytes:
                    pass  # normal partial download
        ...
        if t.error_message:
            lower_err = t.error_message.lower()
            if 'size mismatch' in lower_err:
                issues.append('NOTE: Error mentions size mismatch')
            if 'timeout' in lower_err and 'download error at byte' not in lower_err:
                issues.append('NOTE: Error mentions timeout')
